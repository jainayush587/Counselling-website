...
<tr>
<td>
<button type="submit">Click to find out students with severe problems</button>
</td>
</tr>
</table>
<br><br>
...
include 'db.php';
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $sql = ($conn, "SELECT * FROM student_data WHERE class='$class' AND dept='$dept';");//////here is the problem
	  // class = '$class' and dept = '$dept' group by rollno ";
	  /* elseif (isset($_POST['list'])) {
                $branch = $_POST['branch'];
                $br_name_query = pg_query($conn, "SELECT description FROM branch_master WHERE id=$branch and discipline_id=$discipline_id;");
     */
...
/*include 'db.php';
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $class = mysqli_real_escape_string($conn,$_POST['class']);
      $dept = mysqli_real_escape_string($conn,$_POST['dept']); 
      $sql = "SELECT * FROM student_data WHERE class = '$class'";// group by rollno";//group by rollno
